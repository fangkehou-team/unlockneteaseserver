...
const signParam = async (param) => {
	...
	const {img_key, sub_key} = await cs.cache("wbikey", async () => await getWbiKeys())
...
const getBiliVideoHeader = async () => {
	...
	return cs.cache("bilicookie", () => request('GET', url)
		.then((response) =>
...
const search = (info) => {
	...
	return getBiliVideoHeader().then((cookies) => {
		...
		logger.debug(`bili headers ${cookies}`);
			...
			.then((jsonBody) => {
				if (jsonBody.code === 0) {
					...
					return signParam({
						...
						const url =
							'https://api.bilibili.com/x/player/wbi/playurl?' + param;
						...
						return request('GET', url)
							...
							.then((jsonBody) => {
								if (jsonBody.code === 0) {
									...
									logger.debug(`bilivideo url is ${jsonBody.data.durl[0].url}`);

									return jsonBody.data.durl[0].url;
